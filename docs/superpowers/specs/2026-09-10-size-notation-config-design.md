...
этап 3 (автообучение майнером) — последним.
...
**Уровень C (этап 3: автообучение, полный авто с gate)** —
`tools/size_format_mine.py`:
- агрегирует skipped-отчёты, выделяет поле размера из отклонённых строк,
  группирует/дедуплицирует кластеры (порог устойчивости — в конфиге);
- для устойчивых кластеров вызывает kimi (subprocess, strict JSON) с
  требованием предложить аддитивные варианты записи для
  `size_notations.yaml` — **без единой цифры размера** (LLM выдаёт только
  структуру: класс формата, символы-префиксы/суффиксы, шаблоны вида
  `prefix + digits`);
- патч применяется к КОПИИ конфига и проходит gate: кластер парсится
  каскадом + полный `pytest tests/` зелёный + coverage baseline не падает;
- gate зелёный → коммит в конфиг с provenance (кластер, дата, число
  вхождений, сессия kimi); gate красный → патч отбрасывается, кластер
  остаётся в отчёте;
- ограждения: патчи только аддитивные (не изменяют/не удаляют существующие
  записи), каждый шаг в git для лёгкого revert.
