...
Один проект не представляет разнообразие форматов (в репозитории ≥4 проекта
с разными стилями записи: КР-0324, Шипиловский/МОЭК, КП №1090, ГБОУ №599).
Поэтому корпус трёхуровневый, у каждого уровня своя метрика:

**Уровень baseline (регрессия)** — `tmp/vladik_success.json` (234 позиции,
реальное имя файла — НЕ `vladikavkaz_success.json`) →
`tests/fixtures/vladik_success.json`. Метрика: end-to-end совпадение извлечённых
params с эталонными (проверка цифр). Ловит регрессии рефакторинга.

**Уровень B (синтетическая матрица форматов)** — руками составленный список
вариантов записи (`Ø315, 315ø, Ду315, DN315, Ф315, d315, 1250x800, 1250х800,
1250×800, 315/315/200, ...`) в `tests/fixtures/size_format_matrix.yaml`:
`format_class → list[str]`. Метрика: recall по классам форматов. Не зависит
от выбора проекта.

**Уровень A (мультикорпус проектов)** — одноразовый скрипт прогоняет
существующие экстракторы (`pdf_spec_extractor`, `project_spec_xlsx`) по всем
PDF/xls из `examples/` и собирает сырые строки размеров → снапшот
`tests/fixtures/multi_project_sizes.json`. Эталонных цифр для этих проектов
нет, поэтому метрика — только **покрытие** (доля распознанных строк), без
проверки значений. Точность цифр гарантируется детерминированным извлечением
по spans + валидацией диапазонов.

`tests/test_size_coverage.py` объединяет три уровня:
- baseline: `extract_dimensions(comment)` с выключенным LLM
  (флаг конфига `llm.enabled: false`, дефолт), сравнение params с эталоном;
- матрица B: recall по классам форматов;
- мультикорпус A: recall покрытия по строкам;
- baseline фиксируется числами в тесте; падение ниже = fail;
- тест не ходит в сеть и в LLM.

**Уровень C (процесс, не код)** — строки, отклонённые каскадом и LLM,
попадают в skipped-отчёт; разбор отчётов добывает новые форматы для конфига.
...
3. Новые юнит-тесты: `size_notations` (варианты из конфига),
   матрица форматов B, мультикорпус A,
